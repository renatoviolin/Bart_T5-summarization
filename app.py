...
def bart_summarize(input_text, num_beams=6, num_words=50):
    input_text = str(input_text)
    input_text = ' '.join(input_text.split())
    input_tokenized = bart_tokenizer.encode(input_text, return_tensors='pt')
    summary_ids = bart_model.generate(input_tokenized, num_beams=int(num_beams), max_length=int(num_words), early_stopping=True)
    output = [bart_tokenizer.decode(g, skip_special_tokens=True, clean_up_tokenization_spaces=False) for g in summary_ids]
    return output[0]


def t5_summarize(input_text, num_beams=6, num_words=50):
    ...
    summary_ids = t5_model.generate(input_tokenized, num_beams=int(num_beams), max_length=int(num_words), early_stopping=False)
